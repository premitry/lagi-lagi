...
function bind(haikal) {
    ...
    haikal.ev.on('connection.update', update => {
        Object.assign(state, update)
    })

    haikal.ev.on('chats.set', ({ chats: newChats, isLatest }) => {
        /**if (isLatest) {
            chats.clear()
        }**/
    ...
    haikal.ev.on('contacts.set', ({ contacts: newContacts }) => {
    ...
    haikal.ev.on('messages.set', ({ messages: newMessages, isLatest }) => {
        if (isLatest) {
    ...
    haikal.ev.on('messages.delete', item => {
        if ('all' in item) {
    ...
    haikal.ev.on('chats.delete', deletions => {
        for (const item of deletions) {
            chats.deleteById(item)
        }
    })

    haikal.ev.on('chats.update', updates => {
    ...
    haikal.ev.on('presence.update', ({ id, presences: update }) => {
        presences[id] = presences[id] || {}
        Object.assign(presences[id], update)
    })

    haikal.ev.on('messages.upsert', ({ messages: newMessages, type }) => {
        switch (type) {
            case 'append':
            case 'notify':
                for (const msg of newMessages) {
    ...
    /**haikal.ev.on('messages.update', updates => {
    ...
    haikal.ev.on('groups.update', updates => {
    ...
    haikal.ev.on('group-participants.update', ({ id, participants, action }) => {
    ...
    haikal.ev.on('message-receipt.update', updates => {
    ...
    return {
        ...
        loadMessages: async (jid, count, cursor) => {
            ...
            const retrieve = async (count, cursor) => {
                const result = await (haikal === null || haikal === void 0 ? void 0 : haikal.fetchMessagesFromWA(jid, count, cursor))
                return result || []
            }
        ...
        loadMessage: async (jid, id) => {
            ...
            if (!message) {
                message = await (haikal === null || haikal === void 0 ? void 0 : haikal.loadMessageFromWA(jid, id))
            }
            return message
        },
        mostRecentMessage: async (jid) => {
            ...
            if (!message) {
                const [result] = await (haikal === null || haikal === void 0 ? void 0 : haikal.fetchMessagesFromWA(jid, 1, undefined))
                message = result
            }
            return message
        },
        fetchImageUrl: async (jid) => {
            ...
            if (!contact) {
                return haikal === null || haikal === void 0 ? void 0 : haikal.profilePictureUrl(jid)
            }
            if (typeof contact.imgUrl === 'undefined') {
                contact.imgUrl = await (haikal === null || haikal === void 0 ? void 0 : haikal.profilePictureUrl(jid))
            }
            return contact.imgUrl
        },
        fetchGroupMetadata: async (jid) => {
            if (!groupMetadata[jid]) {
                groupMetadata[jid] = await (haikal === null || haikal === void 0 ? void 0 : haikal.groupMetadata(jid))
            }
            return groupMetadata[jid]
        },
        fetchBroadcastListInfo: async (jid) => {
            if (!groupMetadata[jid]) {
                groupMetadata[jid] = await (haikal === null || haikal === void 0 ? void 0 : haikal.getBroadcastListInfo(jid))
            }
            return groupMetadata[jid]
        },
        fetchMessageReceipts: async ({ remoteJid, id }) => {
            ...
            if (!receipts) {
                receipts = await (haikal === null || haikal === void 0 ? void 0 : haikal.messageInfo(remoteJid, id))
                if (msg) {
